...
importDataServer <- function(id,
  ...
  moduleServer(id,
               function(input, output, session) {
                 ...
                 dataForImport <- selectDataServer("dataSelecter", customNames)
                 ...
                 observeEvent(input$openPopup, ignoreNULL = TRUE, {
                   ...
                   reset("file")
                   dataForImport$warnings <- list()
                   dataForImport$errors <- list()
                   dataForImport$fileName <- ""
                   dataForImport$fileImportSuccess <- NULL
                   dataForImport$dataForImport <- NULL
                   dataForImport$preview <- NULL
                   dataForImport$data <- list()
                   #dataSource(NULL)
                 ...
                 preparedData <- prepareDataServer(
                   "dataPreparer",
                   selectedData = reactive(dataForImport$dataImport),
                   nameOfSelected = reactive(dataForImport$fileName)
                 )

                 observeEvent(preparedData(), {
                   logDebug("Updating preparedData()")
                   #dataForImport$dataImport <- preparedData()
                   dataForImport$preview <-
                     cutAllLongStrings(preparedData(), cutAt = 20)

                   ## Import valid?
                   dataForImport$warnings$import <- list()
                   dataForImport$errors$import <- list()

                   dataForImport <- checkImport(dataForImport,
                                         df = preparedData() %>%
                                           formatForImport(
                                             outputAsMatrix = outputAsMatrix,
                   ...
                   if (isNotValid(dataForImport$errors, dataForImport$warnings, ignoreWarnings)) {
                     shinyjs::disable(ns("addData"), asis = TRUE)
                     shinyjs::disable(ns("accept"), asis = TRUE)
                     dataForImport$fileImportSuccess <- NULL
                   } else {
                     shinyjs::enable(ns("addData"), asis = TRUE)
                     shinyjs::enable(ns("accept"), asis = TRUE)
                     dataForImport$fileImportSuccess <-
                       "Data import successful"
                   }
                 })
                 ...
                 output$warning <-
                   renderUI(tagList(lapply(
                     unlist(dataForImport$warnings, use.names = FALSE), tags$p
                   )))
                 output$error <-
                   renderUI(tagList(lapply(
                     unlist(dataForImport$errors, use.names = FALSE), tags$p
                   )))
                 output$success <-
                   renderText(dataForImport$fileImportSuccess)

                 output$preview <- renderDataTable({
                   req(dataForImport$preview)
                   DT::datatable(
                     dataForImport$preview,
                     filter = "none",
                     selection = "none",
                 ...
                 observeEvent(input$addData, {
                   ...
                   if (dataForImport$fileName %in% names(mergeList())) {
                     tmpMergeList <- mergeList()
                     tmpMergeList[[dataForImport$fileName]] <- tmpData
                 ...
                 observeEvent(input$accept, {
                   ...
                   dataForImport$data[[dataForImport$fileName]] <-
                     preparedData() %>%
                     formatForImport(
                       outputAsMatrix = outputAsMatrix,
                       includeSd = input$includeSd,
                 ...
                 observeEvent(input$acceptMerged, {
                   ...
                   dataForImport$data[["mergedData"]] <- joinedData() %>%
                     formatForImport(
                       outputAsMatrix = outputAsMatrix,
                       includeSd = FALSE,
                       dfNames = customNames
                 ...
                 observeEvent(input$acceptQuery, {
                   ...
                   customNames$withRownames <- FALSE
                   customNames$withColnames <- TRUE
                   dataForImport$data[["queriedData"]] <- queriedData() %>%
                     formatForImport(
                       outputAsMatrix = outputAsMatrix,
                       includeSd = FALSE,
                       dfNames = customNames
